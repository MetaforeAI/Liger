...
Lion's sign-update means every matrix parameter coordinate moves by exactly `±lr` per step regardless of gradient magnitude. Parameters that need *smaller* steps than the nominal LR (because they are near-optimal) do not get them automatically. The LR schedule and any per-organ LR controller must compensate.

In a system with an external per-parameter or per-organ LR controller (e.g., Morpheus's per-organ controller stack with BSC), this is fine — the controller adjusts. In a system without such a controller, Lion-family optimizers can overshoot at the per-parameter level near convergence. This is a known Lion limitation, not Liger-specific.
...
This is the same architectural pattern Morpheus uses at the model level — distinct structural pathways for distinct data regimes, dispatched per-input rather than collapsed into one homogeneous stack. Applied at the optimizer level, it produces an optimizer that **matches its tool to its workpiece** rather than wielding one tool everywhere.
...
### 9.6 In-Production Trace: Morpheus Neo v2.2 RAH Organ

Liger is in **active production use** as the per-organ optimizer for the RAH (Recursive Adaptive Hub) organ of the Morpheus Neo v2.2 architecture, training as of 2026-05-22 on a 22.7M-parameter `micro` configuration. RAH is a mixed-dimensional routing organ with 49 parameter groups total — 20 matrix-shaped (Lion route) and 29 vector- or scalar-shaped (Yogi route) — and 0.99M total parameters. It is exactly the niche §6 of this paper names: mixed dimensionality, well-conditioned matrix gradients (RAH sits downstream of normalized aggregation), and a system-wide preference for warmup-free operation (Morpheus stacks an LR warmup at the controller level, and the per-organ optimizers must not compound it).

The Morpheus stack emits Liger's `get_telemetry()` dict at every logged step. The first 80 steps of the v2.2 run are extracted in `bench/morpheus_v2.2_liger_telemetry.csv` (17 telemetry/loss pairs) and rendered in `bench/morpheus_v2.2_liger_trajectory.png`. The trace validates the analytical claims directly on a real training run rather than a synthetic benchmark.
...
**Loss trajectory.** Mean loss decreases monotonically from `5.74` at step 1 to `5.39` at step 80 (a 6.1% reduction in 80 steps for a 22.7M-param model with `lr_base = 3e-5 → 3e-7` schedule). The trajectory is monotonic with no divergent steps and no rebounds — the optimizer is doing useful work on the RAH organ from step 1.

**What this trace does and does not measure.** Morpheus Neo v2.2 uses a **per-organ optimizer assignment** across 12 distinct organs, each routed to the optimizer that matches its gradient regime. Liger is **on exactly one organ — RAH** — out of the 12. The full config from this run:

    s4lt:radam=0.25M     embedding:lion=0.05M  left_a:muon=1.78M    left_b:yogi=2.91M
    x:shampoo=3.33M      right_a:muon=7.50M    right_b:yogi=1.35M   dih_a:muon=1.23M
    dih_b:muon=2.59M     rah:liger=0.99M       head:lion=0.70M      (+ global controller)

(see line 2 of `logs/neo_v2.2/neo_v2.2_training.log` in the Morpheus repository.) RAH is selected for Liger because it is the only organ that satisfies all three §1.3 conditions simultaneously: mixed dimensionality (20 matrix + 29 vector/scalar params), well-conditioned matrix gradients (RAH sits downstream of normalized aggregation), and a system-level constraint against compounding warmup with the controller-level LR warmup. The other 11 organs receive different optimizers because they live in different gradient regimes:

  - **`s4lt:radam`** — long-tail S4 parameters with bursty variance, *needs* the RAdam cold-start gate.
  - **`embedding:lion`, `head:lion`** — uniform-dimensional matrix-only blocks where Lion's sign-update is sufficient and the Yogi side of Liger would be dead weight.
  - **`left_a, right_a, dih_a, dih_b:muon`** — heavy matrix transforms where the orthogonalization from Muon's NS5 measurably improves convergence.
  - **`left_b, right_b:yogi`** — bias-heavy / variance-sensitive groups where Yogi alone wins.
  - **`x:shampoo`** — the interaction surface with genuinely ill-conditioned cross-branch gradients, where Shampoo's Kronecker preconditioning earns its overhead.

This is the empirical refutation of the "one optimizer covers everything" assumption that §1 critiques. The Morpheus team — operating independently of this paper — converged on a configuration that matches eleven distinct optimizers to eleven distinct gradient regimes. Liger is published as **one tool in that ensemble**, not as a universal optimizer, and the v2.2 trace is the production validation that it does its specific job correctly when deployed to the niche the paper claims for it.
